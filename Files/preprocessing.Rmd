...
# just miami guys, not really necessary line
total <- total %>% subset(PitcherTeam == "MIA_RED")
...
write.csv(cleaned, "cleanedPitcherGames.csv", row.names = FALSE)
```
...
total <- files %>%
  lapply(read_csv) %>%
  bind_rows() %>% 
  mutate(Batter = ifelse(Batter == "Macdonald, Zach",
                         "MacDonald, Zach", Batter))
# removing unecessary tags, pitchouts etc
total <- total %>% subset(TaggedPitchType != "Other")
total <- total %>% subset(TaggedPitchType != "Knuckleball")
# just miami guys, not really necessary line
total <- total %>% subset(BatterTeam == "MIA_RED")
# remove first names and commas
# total$Pitcher <- str_glue(strsplit(total$Pitcher, ', ')[[1]][2], strsplit(total$Pitcher, ', ')[[1]][1], .sep = ' ')
total$Pitcher <- sub("(\\w+),\\s(\\w+)","\\2 \\1", total$Pitcher)
# order pitches so you dont have to later
...
write.csv(total, "cleanedBatterGames.csv", row.names = FALSE)
```
