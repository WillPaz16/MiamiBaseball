library(dplyr)
library(readr)
files <- list.files('/Volumes/My Passport/All_Data_05',
                    recursive = TRUE)
...
con <- dbConnect(RSQLite::SQLite(),'college_baseball_05.sqlite')


dbWriteTable(conn = con, "trackman_data-05", value = tm_data_final, overwrite = F, append = T)
...
dbGetQuery(con, "SELECT * FROM 'trackman_data-05' LIMIT 10")

write_csv(tm_data_final, "2023 May Data.csv")
